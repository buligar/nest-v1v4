...
axis_handles[0].set_ylabel('Перекрестная спектральная\nплотность (mV**2/Hz)', fontsize=14)
axis_handles[0].set_xlabel("Частота (Hz)", fontsize=14)
axis_handles[0].set_title('Перекрестная спектральная плотность', fontsize=16)
axis_handles[0].set_xlim((0, 100))
...
axis_handles[1].set_title("Попарная причинность по Грейнджеру", fontsize=16)
axis_handles[1].plot(
    connectivity.frequencies,
...
y = [0.05] * len(x)
axis_handles[1].plot(x, y, label='y = 0.05', linestyle='-.')
axis_handles[1].set_ylim((0, 1))
axis_handles[1].set_xlim((0, 100))
axis_handles[1].set_xlabel("Частота (Hz)", fontsize=14)
axis_handles[1].legend(fontsize=12)

axis_handles[2].set_title("Частичная направленная когерентность", fontsize=16)
axis_handles[2].plot(
    connectivity.frequencies,
...
axis_handles[2].set_xlim((0, 100))
axis_handles[2].set_xlabel("Частота (Hz)", fontsize=14)
